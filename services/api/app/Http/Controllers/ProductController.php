...
class ProductController extends Controller
{
    ...
    public function show($id)
    {
        ...
        $productArray['variants'] = $product->variants->map(function ($variant) {
            ...
            // 2. Quantités en réservation (réservations non finalisées) — via batches FIFO
            //    (l'ancienne lecture $variant->reserved_quantity n'était jamais alimentée)
            $reservedQuantity = (int) DB::table('reservation_item_batches as rib')
                ->join('reservation_items as ri', 'rib.reservation_item_id', '=', 'ri.id')
                ->join('reservations as r', 'ri.reservation_id', '=', 'r.id')
                ->where('ri.variant_id', $variant->id)
                ->where('rib.status', 'reserved')
                ->whereIn('r.status', ['pending', 'confirmed', 'partial_paid'])
                ->where('r.expiry_date', '>', now())
                ->sum('rib.quantity');

            // 3. Quantités en crédit actif (crédits non payés complètement) — via batches FIFO
            $creditQuantity = (int) DB::table('credit_item_batches as cib')
                ->join('credit_items as ci', 'cib.credit_item_id', '=', 'ci.id')
                ->join('credits as c', 'ci.credit_id', '=', 'c.id')
                ->where('ci.variant_id', $variant->id)
                ->where('cib.status', 'sold')
                ->whereIn('c.status', ['active', 'partial_paid', 'overdue'])
                ->sum('cib.quantity');

            // 4. Quantités vendues via réservations finalisées
            $soldViaReservation = (int) DB::table('reservation_item_batches as rib')
                ->join('reservation_items as ri', 'rib.reservation_item_id', '=', 'ri.id')
                ->join('reservations as r', 'ri.reservation_id', '=', 'r.id')
                ->where('ri.variant_id', $variant->id)
            ...
            $soldViaCredit = (int) DB::table('credit_item_batches as cib')
                ->join('credit_items as ci', 'cib.credit_item_id', '=', 'ci.id')
                ->join('credits as c', 'ci.credit_id', '=', 'c.id')
                ->where('ci.variant_id', $variant->id)
    ...
    public function getVariantBatches(Request $request, $productId, $variantId)
    {
        ...
        // Quantité réellement réservée par batch, via les batches de
        // réservations actives (source de vérité unique — la colonne
        // stock_batches.reserved_quantity n'est plus alimentée).
        $reservedByBatch = DB::table('reservation_item_batches as rib')
            ->join('reservation_items as ri', 'rib.reservation_item_id', '=', 'ri.id')
            ->join('reservations as r', 'ri.reservation_id', '=', 'r.id')
            ...
            ->whereIn('r.status', ['pending', 'confirmed', 'partial_paid'])
            ->where('r.expiry_date', '>', now())
            ->selectRaw('rib.batch_id, SUM(rib.quantity) as reserved')
            ->pluck('reserved', 'rib.batch_id');
        ...
        foreach ($allBatches as $batch) {
            $batch->reserved_quantity = (int) ($reservedByBatch[$batch->id] ?? 0);
    ...
    public function showWithVariants($id)
    {
        ...
        // NB : reserved_quantity (colonne fantôme) remplacée par la somme réelle
        // des batches de réservations actives (source de vérité unique).
        ...
        // Formatter les variantes pour le frontend
        $variants = $product->variants->map(function ($variant) use ($reservedByVariant) {
            ...
            return [
                ...
                'reserved_quantity' => (int) ($reservedByVariant[$variant->id] ?? 0),
                'low_stock_threshold' => $variant->low_stock_threshold,
                'image_path' => $variant->image_path,
                'is_active' => $variant->is_active,
    ...
    public function update(Request $request, $id)
    {
        ...
        try {
            ...
            if ($request->has('image_url')) {
                $data['image_url'] = $request->image_url ?: null;
            }
