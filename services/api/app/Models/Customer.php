...
class Customer extends Model
{
    ...
    /**
     * Retire les points de fidélité gagnés sur un achat (annulation de vente,
     * de crédit ou de réservation). Le solde ne peut pas devenir négatif.
     */
    public function revokePurchase(float $amount): void
    {
        ...
        $toRemove = min($points, $this->loyalty_points);
        if ($toRemove > 0) {
            $this->decrement('loyalty_points', $toRemove);

            $note = sprintf(
                '[%s] -%d points: Annulation d\'un achat de %s Ar',
                now()->format('Y-m-d H:i'),
                $toRemove,
                $amount
            );
